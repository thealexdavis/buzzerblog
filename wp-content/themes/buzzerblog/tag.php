<?php get_header(); 
	...
	$cat_count = get_tag($category->term_id);
	$totalCount = $cat_count->count;
	
	$url_path = trim(parse_url(add_query_arg(array()), PHP_URL_PATH), '/');
	$offset = 1;
	if(strpos($url_path, "tag/".$cat_count->slug."/") !== false ){
		$offset = str_replace("tag/".$cat_count->slug."/page/", "", $url_path);
	}
	$offsetNum = $offset - 1;
		        ...
		        <div class="container">
				    <div class="the_post">
					    ...
					    <div class="pagination">
						    <a class="new <?php if($offsetNum < 1){ echo 'disabled'; }; ?>" <?php if($offsetNum < 1){ echo 'disabled'; }; ?> <?php if($offsetNum >= 1){ ?>href="/tag/<?php echo $cat_count->slug; ?>/page/<?php echo $offsetNum; ?>/"<?php } ?>><i class="fas fa-chevron-left"></i> Newer</a>
						    <a class="old <?php if(($offset*10) >= $totalCount){ echo 'disabled'; }; ?>" <?php if(($offset*10) >= $totalCount){ echo 'disabled'; }; ?> <?php if(($offset*10) < $totalCount){ ?>href="/tag/<?php echo $cat_count->slug; ?>/page/<?php echo $offsetNum + 2; ?>/"<?php } ?>>Older <i class="fas fa-chevron-right"></i></a>
					    </div>
					    </div>
					    <div class="sidebar">
						    <div class="popular">
